...
export const FiltrosMetricas = ({ filtros, empresas, onChange }: Props) => (
  <div className="flex flex-wrap gap-3">
    ...
    <select
      ...
      className="text-xs border border-zinc-200 rounded-lg px-3 py-2 bg-white text-zinc-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
    >
    ...
    <select
      ...
      onChange={(e) => onChange({
        ...
        sub_plataforma: "", // reset sub al cambiar plataforma
      })}
      className="text-xs border border-zinc-200 rounded-lg px-3 py-2 bg-white text-zinc-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
    >
      {PLATAFORMAS.map((p) => (
