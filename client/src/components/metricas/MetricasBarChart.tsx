/** src/components/metricas/MetricasBarChart.tsx — AJUSTE NEON (radar)
 * El radar YA usaba useChartColors() ✓. Único fix: PolarGrid stroke="#e4e4e7" (gris claro)
 * y ticks claros → grid/labels oscuros para que se vea en el theme. Lógica/datos INTACTOS.
 */
...
const PLATAFORMA_TONO: Record<string, string> = { meta: "p1", google: "accent", tiktok: "axis" };
  ...
  return (
    <div className={`${TOOLTIP_BASE} px-3 py-2 text-xs space-y-1`}>
      ...
      {payload.map((p: any, i: number) => (
        <p key={i} style={{ color: p.color }}>{p.name}: {p.value.toLocaleString("es-PE")}</p>
      ))}
    </div>
  );
};
...
export const MetricasBarChart = ({ metricas }: Props) => {
  ...
  if (!plataformas.length) return (
    <div className="flex items-center justify-center h-40 text-xs text-zinc-500">Sin datos para graficar</div>
  );
    ...
    <div className={CARD_CLASS}>
      ...
      {mounted && <ResponsiveContainer width="100%" height={240}>
        <RadarChart data={radarData} margin={{ top: 10, right: 30, bottom: 10, left: 30 }}>
          ...
          <PolarAngleAxis dataKey="metric" tick={{ fontSize: 11, fill: "#a1a1aa", fontWeight: 500 }} />
          <Tooltip content={<TooltipRadar />} />
          {plataformas.map(p => (
            <Radar filter="url(#neon-glow)" key={p} name={LABELS[p] ?? p} dataKey={p}
              stroke={platColor(p)} fill={platColor(p)} fillOpacity={0.15} strokeWidth={2} />
          ))}
