...
const TooltipBar = ({ active, payload }: any) => {
  ...
  return (
    <div className={`${TOOLTIP_BASE} p-2.5 text-xs`}>
      <p className="font-semibold text-zinc-200 truncate max-w-[200px]">{d.label ?? d.motivo}</p>
      <p className="text-zinc-400 mt-0.5">{d.total} ocurrencia{d.total !== 1 ? "s" : ""}</p>
    </div>
  );
};
          ...
          <button
            ...
            className={`px-3 py-1 text-xs rounded-md transition ${
              capa === "primer_contacto"
                ? "bg-slate-800/60 shadow-sm text-zinc-200 font-medium"
                  ...
                  <div key={e.etapa} className="flex items-center gap-2">
                    ...
                    <div className="flex-1 bg-zinc-800 rounded-full h-2">
                      <div
                        ...
                        style={{ width: `${Math.max(pct, 2)}%`, background: color, boxShadow: `0 0 6px ${color}` }}
                      />
                    </div>
                    <span className="text-[10px] font-semibold text-zinc-400 w-8 text-right">{e.total}</span>
                  </div>
                );
              })
            )}
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-white/8">
                  <th className="text-left py-1.5 text-zinc-100 font-medium">Etapa</th>
                  <th className="text-left py-1.5 text-zinc-100 font-medium">Motivo</th>
                  <th className="text-right py-1.5 pr-1 text-zinc-100 font-medium">Leads</th>
                </tr>
              </thead>
              <tbody>
                {data.cruce.slice(0, 8).map((c, i) => (
                  <tr key={i} className="border-b border-white/5">
                    ...
                    <td className="py-1.5 pr-1 text-right font-semibold text-zinc-300">{c.total}</td>
                  </tr>
                ))}
              </tbody>
            </table>
