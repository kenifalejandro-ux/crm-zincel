    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className={`${MODAL_BASE} w-full max-w-md`}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="text-xs font-medium text-zinc-300 block mb-1">Nombre *</label>
            <input
              value={form.nombre}
              onChange={e => set("nombre", e.target.value)}
              placeholder="ej. RL Safety SAC"
              className={`${INPUT_BASE} w-full px-3 py-2 text-xs focus:outline-none focus:border-zinc-400`}
              required
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-zinc-300 block mb-1">Sector</label>
              <select
                value={form.sector}
            ...
            <div>
              <label className="text-xs font-medium text-zinc-300 block mb-1">Moneda</label>
              <select
                value={form.moneda}
          ...
          <div>
            <label className="text-xs font-medium text-zinc-300 block mb-1">Notas</label>
            <textarea
              value={form.notas}
