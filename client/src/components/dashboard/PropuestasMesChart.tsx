...
export function PropuestasMesChart({ anio }: Props) {
  ...
  if (cargando) return (
    <div className="bg-white rounded-2xl border border-zinc-100 p-5 h-48 flex items-center justify-center">
    ...
    <div className="bg-white rounded-2xl border border-zinc-100 p-5 shadow-[0_1px_3px_rgba(0,0,0,0.06),_0_4px_16px_rgba(0,0,0,0.04)]">
      ...
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={chartData} barSize={14} barGap={2}
          ...
          />
          <Bar dataKey="Enviadas"    stackId="a" fill={COLORES.enviadas}       radius={[0,0,0,0]} />
          <Bar dataKey="Negociación" stackId="a" fill={COLORES.en_negociacion} radius={[0,0,0,0]} />
          <Bar dataKey="Ganadas"     stackId="a" fill={COLORES.ganadas}        radius={[0,0,0,0]} />
          <Bar dataKey="Perdidas"    stackId="a" fill={COLORES.perdidas}       radius={[4,4,0,0]} />
        </BarChart>
      </ResponsiveContainer>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1">
            {chartData.filter(d => d.total > 0).map(d => (
              <span key={d.mes} className="text-[11px] text-zinc-500">
                <span className="font-semibold text-zinc-700">{d.mes}</span>: {d.total}
