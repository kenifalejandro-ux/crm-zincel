...
const TooltipPersonalizado = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-lg shadow-sm p-3 text-xs">
      ...
      {d.total > 0 && (
        <p className="text-zinc-700 mt-1">Tasa: {Math.round((d.contestadas / d.total) * 100)}%</p>
      )}
    </div>
  );
};
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <div className={CARD_CLASS}>
        ...
        {totalItems > 0 && (
          <p className="text-[11px] text-zinc-600 font-medium mb-3">
            {totalItems} {labelPeriodo}{totalItems !== 1 ? "s" : ""} · línea de tendencia
          </p>
        )}

        {estadisticas.length === 0 ? (
          <p className="text-xs text-zinc-600 text-center py-10">Sin llamadas en este período</p>
        ) : (
