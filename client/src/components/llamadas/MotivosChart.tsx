            ...
            return (
              <div key={d.motivo_perdida}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-zinc-400">{LABELS[d.motivo_perdida] ?? d.motivo_perdida}</span>
                  <span className="text-xs font-semibold text-zinc-300">{d.total} <span className="text-zinc-400 font-normal">({pct}%)</span></span>
                </div>
                <div className="w-full bg-zinc-800 rounded-full h-1.5">
                  <div className="h-1.5 rounded-full transition-all" style={{ width: `${pct}%`, background: color, boxShadow: `0 0 6px ${color}` }} />
                </div>
              </div>
            );
          })}
